  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', paddingBottom: 80 }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: '#fff', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 4px rgba(0,0,0,0.1)' }}>
        ...
        <span onClick={handleGenerateLink} style={{ color: '#1677ff', cursor: 'pointer' }}>分享</span>
      </div>
      ...
      <div style={{ background: '#fff', padding: 16, marginTop: 8 }}>
        ...
        {product.price && <div style={{ color: '#ff4d4f', fontSize: 24, fontWeight: 'bold', marginBottom: 12 }}>¥{product.price}</div>}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', padding: '12px 16px', boxShadow: '0 -1px 4px rgba(0,0,0,0.1)' }}>
        {visitor?.phone ? (
          <a href={`tel:${visitor.phone}`} style={{ display: 'block', background: '#1677ff', color: '#fff', textAlign: 'center', padding: '12px 0', borderRadius: 8, fontSize: 16 }}>联系客服</a>
        ) : (
          <button onClick={handleGenerateLink} style={{ display: 'block', width: '100%', background: '#1677ff', color: '#fff', textAlign: 'center', padding: '12px 0', borderRadius: 8, fontSize: 16, border: 'none', cursor: 'pointer' }}>获取联系方式</button>
        )}
      </div>
    </div>
  )
