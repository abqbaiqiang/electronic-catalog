...
export default function CatalogHomePage() {
  ...
  const loadData = async (id) => {
    try {
      ...
      const allCategories = await mockDb.getCategoriesByCatalogId(id)
      const filteredCategories = allCategories.filter(c => c.is_visible)
      ...
      const allProducts = await mockDb.getProductsByCatalogId(id)
  ...
  if (showForm && !visitor) {
    return (
      <div style={{ minHeight: '100vh', background: catalog.banner_url ? `url(${catalog.banner_url}) center/cover` : '#1677ff', padding: '60px 20px 40px' }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 24, maxWidth: 400, margin: '0 auto' }}>
          ...
          <form onSubmit={handleFormSubmit}>
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="text" value={formData.company} onChange={(e) => setFormData({...formData, company: e.target.value})} placeholder="请输入公司名称" style={{ width: '100%', padding: '10px 12px', border: '1px solid #d9d9d9', borderRadius: 8, fontSize: 14, boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 6, fontSize: 14 }}>联系人 *</label>
              <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="请输入联系人姓名" style={{ width: '100%', padding: '10px 12px', border: '1px solid #d9d9d9', borderRadius: 8, fontSize: 14, boxSizing: 'border-box' }} />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', marginBottom: 6, fontSize: 14 }}>手机号 *</label>
              <input type="tel" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="请输入联系人手机号" style={{ width: '100%', padding: '10px 12px', border: '1px solid #d9d9d9', borderRadius: 8, fontSize: 14, boxSizing: 'border-box' }} />
            </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', maxWidth: 750, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', minHeight: 'calc(100vh - 180px)' }}>
        <div style={{ width: 80, background: '#fff', overflowY: 'auto', flexShrink: 0 }}>
          {categories.map(category => (
            <div key={category.id} onClick={() => setSelectedCategory(category.id)} style={{ padding: '12px 8px', textAlign: 'center', fontSize: 13, cursor: 'pointer', background: selectedCategory === category.id ? '#f5f5f5' : '#fff', color: selectedCategory === category.id ? '#1677ff' : '#666', borderLeft: selectedCategory === category.id ? '3px solid #1677ff' : '3px solid transparent', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{category.name}</div>
          ))}
        </div>
              ...
              {filteredProducts.map(product => (
                <div key={product.id} onClick={() => handleProductClick(product.id)} style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', cursor: 'pointer' }}>
                  ...
                  <div style={{ padding: 8 }}>
                    ...
                    {product.price != null && <div style={{ color: '#ff4d4f', fontSize: 14, fontWeight: 'bold' }}>{"\u00A5"}{product.price}</div>}
                  </div>
                </div>
              ))}
            </div>
